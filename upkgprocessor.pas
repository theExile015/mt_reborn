...
// Если нашли - устанавливаем координаты, убавляем АП
  ...
  // проверяем на инерцию
  if chars[combats[comLID].Units[uLID].charLID].perks[0][3] > 0 then  // INERTIA
     CM_AddAura(comLID, uLID, 2, PerksDB[7].xyz[chars[combats[comLID].Units[uLID].charLID].perks[1][3]].x * (length(combats[comLID].Way) - 1));
...
begin
  ...
  if pkg.spellID = 14 then
   ...
   if pkg.spellID = 15 then
     begin
       if combats[comLID].Units[pkg.uLID].Data.cAP > 9 then
          begin
            CM_AddAura(comLID, pkg.uLID, 9, 0);
            combats[comLID].On_Recount := true;
          end;
     end;
try
