...
uses uLocalization, u_MM_GUI, uChat, uLoader, uNetCore, uMyGUITT;
         ...
         if i = 12 then
            begin
              ...
              Scissor_Begin( round(mWins[i].rect.X + 24), round(mWins[i].rect.Y + 24), 352, 352);
                dx := locs[activechar.header.loc].data.x - 200;
                dy := locs[activechar.header.loc].data.y - 200;
                if dx < 0 then dx := 0;
                if dy < 0 then dy := 0;
                ...
                for j := 1 to high(locs) do
                  if locs[j].exist then
                     for k := 1 to 25 do
                       if locs[j].data.links[k] <> 0 then
                          if locs[locs[j].data.links[k]].exist then
                             pr2d_line( mWins[i].rect.X + locs[j].data.x - dx,
                                        mWins[i].rect.Y + locs[j].data.y - dy,
                                        mWins[i].rect.X + locs[locs[j].data.links[k]].data.x - dx,
                                        mWins[i].rect.Y + locs[locs[j].data.links[k]].data.y - dy, $00AA00, 200);
                for j := 1 to high(locs) do
                  if locs[j].exist then
                  begin
                     if col2d_PointInRect( mouse_X, mouse_Y, rect(mWins[i].rect.X + locs[j].data.x - dx - 20, mWins[i].rect.Y + locs[j].data.y - dy - 20, 40, 40)) then
                        begin
                          FLAG := FX_BLEND or FX_COLOR;
                          Loc_OMO := j;
                        end else FLAG := FX_BLEND;
                     fx2d_SetColor($f0ac34);
                     ASprite2d_Draw( tex_map_locs, mWins[i].rect.X + locs[j].data.x - dx - 20, mWins[i].rect.Y + locs[j].data.y - dy - 20, 40, 40, 0, locs[j].data.pic, 255, FLAG);
                     if activechar.header.loc = j then color := $f0ac34 else color := $dddddd;
                     Text_DrawEx( fntMain, mWins[i].rect.X + locs[j].data.x - text_GetWidth(fntMain, locs[j].data.name) / 2 - dx,
                                  mWins[i].rect.y + locs[j].data.y + 1 - dy + 20, 1, 0, locs[j].data.name, 255, color);
                  end;
                Scissor_End();
            end;
         // отрисовка линий
           ...
           if mWins[i].dlgs[j].exist then
              begin
                ...
                ASprite2d_Draw( tex_LocIcons, mWins[i].rect.X + 30, mWins[i].rect.Y + mWins[i].dlgs[j].dy,
                                16, 16, 0, mWins[i].dlgs[j].data.dType, 255 );
                Text_DrawEx(fntMain2, mWins[i].rect.X + 55, mWins[i].rect.Y + mWins[i].dlgs[j].dy + 2,
                            1, 0, mWins[i].dlgs[j].data.text, 255, $e09c24);
              end;
        Scissor_End();
       ...
       if igs = igsMap then // селектор локаций
          begin
            if Mouse_Click(M_BLEFT) then
               DoTravel(Loc_OMO);// перемещаться на локацию
          end;
        ...
        if mWins[i].dlgs[j].exist then
           begin
             ...
             if mWins[i].dlgs[j].omo and Mouse_Click(M_BLEFT) then
                dlg_Click(mWins[i].dlgs[j].data.dType, mWins[i].dlgs[j].data.dID);
           end;
...
try
  if File_Exists('Cache\items.rec') then
     begin
       File_Open(Cache, 'Cache\items.rec', FOM_OPENR);

       if file_GetSize(Cache) >= sizeof(data) then
       while file_getpos(Cache) < file_getsize(Cache) do
       begin
        // Writeln(file_getpos(itemcache), '/', file_getsize(itemcache));
         file_Read(Cache, data, sizeof(data));
         if data.ID <> 0 then
            begin
     ...
     end else file_open(Cache, 'Cache\items.rec', FOM_CREATE);
except
...
try
  if File_Exists('Cache\locs.rec') then
     begin
       File_Open(Cache, 'Cache\locs.rec', FOM_OPENR);

       if file_GetSize(Cache) >= sizeof(ldata) then
       while file_getpos(Cache) < file_getsize(Cache) do
       begin
        // Writeln(file_getpos(itemcache), '/', file_getsize(itemcache));
     ...
     end else file_open(Cache, 'Cache\locs.rec', FOM_CREATE);
except
  On e: ERangeError do writeln('ERangeError :: uMyGui :: CacheLoad' );
end;
  File_Close(Cache);
