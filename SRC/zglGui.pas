...
{$ENDIF}
      ...
      procedure ShowMessage(const Caption, Data: TCaption; const Effect: TDisplayEffect = deFadeIn);
...
begin
  ...
  if scissor_Rect(rct, clntRect, nil, sciEnd) then begin
    ...
    text_DrawEx(fnt.Data, round(rct.X + i + off), round(rct.Y + rct.H/2 + 1),
    ...
    if Selection and (Gui.Handler.hObject = self) then begin
      ...
      text_DrawEx(fnt.Data,
        round(rct.X + selStart + selWidth * (1 - f)), round(rct.Y + rct.H/2 + 1),
    ...
    if (Gui.Handler.hObject = self) and (fUpd < 250) then begin
      ...
      pr2d_line(rct.X + selPos, rct.Y, rct.X + selPos, rct.Y+ rct.H,
        {Self.Gui.Skin.SkinCfg[cfgEditLine]}$E6D690, 255);
    end;
    if sciEnd then
      scissor_End;
  end;
...
begin
  ...
  if utf8_Length(self.Caption) > self.MaxLength then
     self.Caption := utf8_copy( self.Caption, 1, self.MaxLength );
end;
...
begin
  ...
  if scissor_Rect(rct, clntRect, nil, sciEnd) then begin
    ...
    if scissor_Rect(rct, clntRect, nil, sciEnd) then begin
      text_DrawInRectEx(fnt.Data, rct, fnt.Size, 0, AnsiToUTF8(Caption),
        fnt.Alpha, fnt.Color, FontAlignes[fnt.Align] or TEXT_CLIP_RECT);
      if sciEnd then
        scissor_End;
    end;
...
begin
  ...
  if (Items.Count = 0) then begin
    ...
    Caption := AnsiToUTF8(TCaption(Items[Selected]));
  end;
      ...
      if scissor_Rect(rct, @rct, nil, sciEnd) then begin
        ...
        for i := 0 to ic - 1 do begin
          ...
          text_DrawInRectEx(fnt.Data, dr, fnt.Size, 0,
            AnsiToUTF8(TCaption(Items[i + fOffset])), fnt.Alpha, fnt.Color,
...
procedure zglTButton.Draw;
 function _rct(x, y, w, h: single) : zglTRect;
 begin
   result.X:=x;
   result.Y:=y;
   result.W:=w;
...
begin
  ...
  if scissor_Rect(rct, clntRect, nil, sciEnd) then begin
    ...
    text_DrawInRectEx(fnt.Data, _rct(rct.X, rct.Y + 1, rct.W, rct.H), fnt.Size,
...
procedure zglTGui.ShowMessage(const Caption, Data: TCaption; const Effect: TDisplayEffect = deFadeIn);
var f: zglTForm;
    cpt: zglTLabel;
    ok: zglTButton;
    tW, tH: single;
