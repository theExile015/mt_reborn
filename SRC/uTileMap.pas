...
procedure DrawTiles;
var
  i, i2, i3, c, c2: integer; x, y : single; flag : boolean;
begin
...
//  if flag then Exit;
    for i3 := 0 to Length(layer)-1 do
      begin
      //  if flag then Exit;
        if length(layer[i3].tile) < 1 then exit;
        c:= 0;
                  ...
                  begin
                    ...
                    if (not(layer[i3].tile[c].index = 0)) {and (not flag)} then
                       begin
                         ...
                         if (layer[i3].tile[c].index > 672) and (layer[i3].tile[c].index <= 1184) then
                             asprite2d_Draw(texTiles, x, y, 128  + 1, 64 + 1,
                                                      0, layer[i3].tile[c].index - 672);
                         if (layer[i3].tile[c].index > 1376) and (layer[i3].tile[c].index <= 2000) then
                            asprite2d_Draw(texTiles3, x, y, 128  + 1, 64 + 1,
                                                      0, layer[i3].tile[c].index - 1376);
                         if (layer[i3].tile[c].index > 1184) and (layer[i3].tile[c].index <= 1376) then
