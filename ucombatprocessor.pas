              ...
              for j := 0 to high(combats[i].Units) do
                if combats[i].Units[j].uLID = combats[i].NextTurn then
                   begin
                     ...
                     CM_DelAura(i, j, 9); // удаляем деф стойку, если была
                     CM_DelAura(i, j, 2); // удаяляем инерцию, если была
                     CM_Effect2(i, j, 20); // Медитация (если есть)
                   end;
  ...
  for i := 0 to high(combats[comLID].Units) do
    if not combats[comLID].Units[i].exist then
      begin
        ...
        writeln('BVAL = ', combats[comLID].units[i].bVal);
        ...
        combats[comLID].units[i].spow  := chars[charLID].Stats.SPD;
        combats[comLID].Units[i].spi   := chars[charLID].Stats.Spi;
...
begin
  ...
  if auID <> high(byte) then
     begin
       add_Dam := trunc(combats[comLID].units[uLID].auras[auID].stacks * combats[comLID].units[uLID].APH / 10);
...
procedure CM_Effect(comLID, uLID, e_ID: DWORD);
var i, r, dmg, deadly: integer;
    dr       : single;
    charLID, _charLID : DWORD;
    _head    : TPackHeader; _pkg : TPkg116;
    mStr     : TMemoryStream;
begin
  dr := 0; deadly := 0;

  for i := 1 to 8 do
    _pkg.victims[i].uLID := 1000;
  ...
  if e_ID = 4 then
     begin  // rend
       ...
       _pkg.victims[1].deadly := deadly;
       _pkg.victims[1].dmg    := dmg;
       _pkg.victims[1].hp_left:= combats[comLID].units[uLID].Data.cHP;
       _pkg.victims[1].uLID   := uLID;
     end;
...
begin
  ...
  Writeln(' ************ CM_Effect_2 ******************* ', e_ID );

  _charLID := combats[comLID].Units[uLID].charLID;
  if e_ID = 8 then
     begin  // inner fire
       _pkg.fail_code := 1;
       r := CM_FindAura(comLID, uLID, 8);
       if r = high(byte) then Exit;

       dmg := Combats[comLID].units[uLID].auras[r].stacks;

       inc(combats[comLID].units[uLID].Data.cHP, dmg);
       if combats[comLID].units[uLID].Data.cHP > combats[comLID].units[uLID].Data.mHP then
          combats[comLID].units[uLID].Data.cHP := combats[comLID].units[uLID].Data.mHP;
  ...
  if e_ID = 10 then
     begin  // vampirism
       r := CM_FindAura(comLID, uLID, 10);
       if r = high(byte) then Exit;

       dmg := Combats[comLID].units[uLID].auras[r].stacks;

       inc(combats[comLID].units[uLID].Data.cHP, dmg);
       CM_DelAura(comLID, uLID, 10);
       if combats[comLID].units[uLID].Data.cHP > combats[comLID].units[uLID].Data.mHP then
          combats[comLID].units[uLID].Data.cHP := combats[comLID].units[uLID].Data.mHP;
  ...
  if e_ID = 20 then
     begin  // meditation
       writeln('cp1');
       if combats[comLID].Units[uLID].uType = 2 then Exit;
       writeln('cp2');
       if chars[combats[comLID].Units[uLID].charLID].perks[2][3] = 0 then Exit;
       writeln('cp3');
       _pkg.fail_code := 2;
       dmg := round(chars[combats[comLID].Units[uLID].charLID].Stats.MPReg * PerksDB[9].xyz[chars[combats[comLID].Units[uLID].charLID].perks[2][3]].x / 100);
       Writeln(' MEDITATION +++ ', dmg);
       inc(combats[comLID].units[uLID].Data.cMP, dmg);
       if combats[comLID].units[uLID].Data.cMP > combats[comLID].units[uLID].Data.mMP then
          combats[comLID].units[uLID].Data.cMP := combats[comLID].units[uLID].Data.mMP;

       _pkg.victims[1].dmg    := dmg;
       _pkg.victims[1].hp_left:= combats[comLID].units[uLID].Data.cMP;
       _pkg.victims[1].uLID   := uLID;
     end;
